...
class BaseModel {
    ...
    /* where with limit,offset & orderby*/
    fetchData(tableName = this.table,query = {},select=[], orderby, order,limit='',offset=0) {

        var prepareQuery = knex(tableName)
        .select(select);
        if(query)
        {
